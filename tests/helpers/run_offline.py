"""Run a local verification process with kernel-enforced network denial.

Linux-only verification helper. Fails closed if the filter cannot
be installed. Does not modify the installed packages or fake network replies.
"""
import ctypes
import errno
import os
...
if len(sys.argv) < 2:
    raise SystemExit('Usage: run_offline.py PROGRAM [ARGUMENTS]')
