...
test("Drive selection opens matching SQL results and a failed selection leaves them identifiable", async ({ page }) => {
  ...
  await page.route("https://www.googleapis.com/drive/v3/files**", async (route) => {
    ...
    if (url.searchParams.get("alt") === "media") {
      if (url.pathname.endsWith("/good-file")) {
        await route.fulfill({ headers, contentType: "text/csv", body: "currency_code,mid_rate\nTEST,1.23\n" });
      } else {
    ...
    else if (q.includes("'fixture-bronze' in parents")) files = [
      { id: "good-folder", name: "fixture_rates" },
      { id: "bad-folder", name: "unavailable_rates" },
    ];
    else if (q.includes("'good-folder' in parents")) files = [{ id: "good-file", name: "data.csv", mimeType: "text/csv", size: "38" }];
    else if (q.includes("'bad-folder' in parents")) files = [{ id: "bad-file", name: "data.csv", mimeType: "text/csv", size: "38" }];
  ...
  await expect(page.getByRole("status").filter({ hasText: "Catalog loaded" })).toBeVisible({ timeout: 60000 });
  ...
  await expect(page.locator(".monaco-editor .view-lines").first()).toContainText('"02_bronze"."fixture_rates"');
  const tabCount = await page.getByRole("tab").count();

  await page.getByText("unavailable_rates", { exact: true }).click();
  await expect(page.getByRole("status").filter({ hasText: "Error loading 'unavailable_rates'" })).toBeVisible();
