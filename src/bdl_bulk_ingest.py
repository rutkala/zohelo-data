...
def _convert_csv(
    ...
    *,
    ...
    with duckdb.connect() as connection:
        source = (
            f"read_csv({csv_sql}, delim=';', header=true, quote='\"', escape='\"', "
            "all_varchar=true, strict_mode=true, null_padding=false)"
        )
    ...
    *,
    ...
    if existing:
        ...
        if (
            ...
            or item.get("md5Checksum") != md5_hex
            or int(item.get("size", -1)) != local_path.stat().st_size
        ):
    ...
    if (
        ...
        or int(response.get("size", -1)) != local_path.stat().st_size
    ...
    *,
    ...
    response = storage.drive_service.files().create(
        body={
            "name": name,
            "parents": [parent_id],
            "appProperties": {"sha256": digest, "kind": "manifest", "source_id": "gus_bdl", "transport": "web_bulk"},
        },
        media_body=media,
        fields="id,name,size,md5Checksum,appProperties",
...
def ingest_archive(archive: Path, subgroup_id: str, allow_production_write: bool) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="zohelo-bdl-bulk-") as temporary:
        ...
        snapshot_root = storage.get_or_create_nested_folder(
            ["gus_bdl", "web_bulk", subgroup_id, archive_sha],
            root_id=landing_root,
        ...
        archive_object = _upload_file(
            ...
            archive,
            name="source.zip",
            parent_id=snapshot_root,
            sha256_hex=archive_sha,
            md5_hex=archive_md5,
            kind="source_zip",
        )
        parquet_object = _upload_file(
            storage,
            parquet_path,
            name="data.parquet",
            parent_id=snapshot_root,
            sha256_hex=converted["parquet_sha256"],
            md5_hex=converted["parquet_md5"],
            kind="source_parquet",
        )
        manifest = {
            "format_version": 1,
            ...
            "subgroup_id": subgroup_id,
            "retrieved_at_utc": retrieved_at,
            "archive_original_filename": archive.name,
            "archive_sha256": archive_sha,
            "archive_bytes": archive.stat().st_size,
            **zip_metadata,
            "row_count": converted["row_count"],
            "source_columns": converted["source_columns"],
    ...
    result = {
        ...
        "archive_bytes": archive.stat().st_size,
        "row_count": converted["row_count"],
        "parquet_bytes": converted["parquet_bytes"],
        "drive_folder_id": snapshot_root,
        "archive_object": archive_object,
        "parquet_object": parquet_object,
