...
The official API is used only for lightweight subject hierarchy discovery. Durable
progress lives in the production Landing Drive tree:

* ``01_landing/gus_bdl/web_bulk/Pxxxx`` means a subgroup has landed successfully.
* ``01_landing/gus_bdl/web_bulk/_control/Pxxxx.json`` means the subgroup was
  inspected and intentionally assigned away from Web bulk (for example, because
  its complete dimension selection is below the 3,500-information bulk threshold).
"""
...
def _bulk_roots(storage: StorageManager) -> tuple[str, str]:
    ...
    bulk = storage.get_or_create_nested_folder(
        ["gus_bdl", "web_bulk"], root_id=landing, write_session=session
    )
    control = storage.get_or_create_nested_folder(
        ["_control"], root_id=bulk, write_session=session
    )
    return bulk, control
...
def _durable_status(storage: StorageManager, bulk_id: str, control_id: str) -> tuple[set[str], set[str]]:
    ...
    processed = {
        ...
        and _SUBGROUP_RE.fullmatch(item["name"].removesuffix(".json"))
    }
...
def _next_subgroup(
    ...
    """Depth-first traversal that stops as soon as one unprocessed P leaf is found."""
        ...
        if identifier.startswith("P") and identifier not in excluded:
            ...
            if k_id and g_id:
                return {
                    ...
                    "subgroup_name": item.get("name"),
                    "category_id": k_id,
                    "group_id": g_id,
                    "url": (
                        "https://bdl.stat.gov.pl/bdl/dane/podgrup/wymiary/"
                        f"{_number(k_id)}/{_number(g_id)}/{_number(identifier)}"
                    ),
                    "path": list(path),
                    "has_variables": item.get("hasVariables"),
                }
        children = item.get("children")
        if not children:
            return None
        child_items = _subjects(session, identifier)
...
def plan() -> dict[str, Any]:
    ...
    return {
        "status": "candidate" if candidate else "complete",
        "landed_subgroups": len(landed),
        "web_checked_nonbulk_subgroups": len(processed),
        "candidate": candidate,
...
def mark(subgroup_id: str, status: str, detail: str | None = None) -> dict[str, Any]:
    ...
    found = storage.drive_service.files().list(
