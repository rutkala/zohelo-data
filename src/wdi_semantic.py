"""Grain-preserving WDI archive and modeled-value coverage metrics."""
...
METRICS = {
    item.name: item
    for item in (
        MetricSpec("wdi_current_archive_total", "current_archive_total"),
        MetricSpec("wdi_archive_member_total", "archive_member_total"),
        MetricSpec("wdi_source_geography_total", "source_geography_total"),
        MetricSpec("wdi_source_indicator_total", "source_indicator_total"),
        MetricSpec("wdi_source_value_total", "source_value_total"),
        MetricSpec("wdi_modeled_geography_total", "modeled_geography_total"),
        MetricSpec("wdi_modeled_indicator_total", "modeled_indicator_total"),
        MetricSpec("wdi_modeled_observation_total", "modeled_observation_total"),
        MetricSpec("wdi_modeled_value_coverage_ratio", "modeled_value_coverage_ratio"),
    )
}
_RATIO_COLUMNS = {"modeled_value_coverage_ratio"}
...
def _spec(metric: str, group_by=None) -> MetricSpec:
    ...
    if group_by is not None and tuple(group_by) != WDI_GRAIN:
        raise ValueError(f"{metric} requires the exact archive snapshot grain {','.join(WDI_GRAIN)}")
...
def _read_manifest(path):
    ...
    if set(METRICS) - names:
        raise ValueError("The semantic manifest does not contain every supported WDI coverage metric")
    models = {item["name"]: item for item in document.get("semantic_models", [])}
    node = models.get("wdi_coverage", {}).get("node_relation", {})
    if node.get("schema_name") != "04_gold" or node.get("alias") != "mart_wdi_coverage":
        raise ValueError("The semantic manifest has an unsupported WDI coverage relation")
    alias = node.get("database")
    if not isinstance(alias, str) or not re.fullmatch(r"[A-Za-z_][A-Za-z_0-9]*", alias):
        raise ValueError("WDI semantic relations must use one simple local database alias")
    return document, alias


def validate_metric_grain(database):
    relation = '"04_gold"."mart_wdi_coverage"'
    with duckdb.connect(str(Path(database).resolve()), read_only=True) as connection:
        if connection.execute(f"select count(*) from {relation} where snapshot_date is null").fetchone()[0]:
            raise ValueError("Invalid WDI coverage metric grain")
        if connection.execute(
            f"select count(*) from (select snapshot_date from {relation} group by snapshot_date having count(*) <> 1)"
        ).fetchone()[0]:
            raise ValueError("Duplicate WDI coverage metric grain")
        for column in {item.column for item in METRICS.values()} - _RATIO_COLUMNS:
            if connection.execute(f"select count(*) from {relation} where {column} is null or {column} < 0").fetchone()[0]:
                raise ValueError(f"Invalid WDI coverage metric value: {column}")
        for column in _RATIO_COLUMNS:
            if connection.execute(f"select count(*) from {relation} where {column} is null or {column} < 0 or {column} > 1").fetchone()[0]:
                raise ValueError(f"Invalid WDI coverage metric ratio: {column}")
...
def _run_cli(project, arguments):
    ...
    for name in tuple(env):
        if name.startswith(("GOOGLE_", "GCP_", "AWS_", "AZURE_")) or name in {
            ...
            env.pop(name, None)
    env.update(
        DBT_PROFILES_DIR=str(project), DBT_PROJECT_DIR=str(project),
        DBT_SEND_ANONYMOUS_USAGE_STATS="false", DO_NOT_TRACK="1", TMPDIR=str(project),
    )
...
def _prepare_project(project, database, semantic_manifest, alias):
    ...
    (project / "dbt_project.yml").write_text(yaml.safe_dump({
        "name": "zohelo_wdi_query", "version": "1.0.0", "config-version": 2,
        "profile": "zohelo_wdi_query", "model-paths": [],
    }), encoding="utf-8")
    (project / "profiles.yml").write_text(yaml.safe_dump({
        "zohelo_wdi_query": {"target": "local", "outputs": {"local": {
            "type": "duckdb", "path": str(project / "query.duckdb"), "database": alias,
            "schema": "main", "threads": 1, "keep_open": False,
...
def _query(database, semantic_manifest, spec, start, end, output_csv, alias):
    ...
    with tempfile.TemporaryDirectory(prefix="zohelo-wdi-metricflow-", dir=output_csv.parent) as temporary:
        project = Path(temporary)
        _prepare_project(project, database, semantic_manifest, alias)
        candidate = project / "result.csv"
        execution = _run_cli(project, [
            "query", "--metrics", spec.name, "--group-by", ",".join(WDI_GRAIN),
        ...
        if not candidate.exists():
            with duckdb.connect(str(database), read_only=True) as connection:
                count = connection.execute(
                    'select count(*) from "04_gold"."mart_wdi_coverage" where snapshot_date between ? and ?',
                    [start, end],
                ).fetchone()[0]
...
def validate_release_metrics(database, semantic_manifest, output_dir):
    ...
    validate_metric_grain(database)
    report = {
        "status": "verified", "engine": "native_metricflow",
        "query_interface": "src/wdi_semantic.py", "grain_validation": "all_gold_rows",
        "comparison_window": "latest_archive_snapshot_date", "metrics": [],
    }
    with duckdb.connect(str(database), read_only=True) as connection:
        dates = [row[0] for row in connection.execute(
            'select distinct snapshot_date from "04_gold"."mart_wdi_coverage" order by snapshot_date desc limit 1'
        ).fetchall()]
    if not dates:
        raise ValueError("No published WDI coverage snapshot is available")
    start = end = dates[0]
    for spec in METRICS.values():
        with duckdb.connect(str(database), read_only=True) as connection:
            expected = [(row[0].isoformat(), Decimal(str(row[1]))) for row in connection.execute(
                f'select snapshot_date, {spec.column} from "04_gold"."mart_wdi_coverage" where snapshot_date between ? and ?',
                [start, end],
            ).fetchall()]
        ...
        report["metrics"].append({
            "name": spec.name, "start_date": start.isoformat(), "end_date": end.isoformat(),
            "row_count": len(rows), "group_by": list(WDI_GRAIN), "matches_gold": True,
        })
