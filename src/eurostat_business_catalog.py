"""Build the release-bound business catalogue for the EUROSTAT modeled platform."""
from __future__ import annotations

from copy import deepcopy
from typing import Any, Mapping

from eurostat_semantic import METRICS as EUROSTAT_METRIC_SPECS

EUROSTAT_METRIC_NAMES = frozenset(EUROSTAT_METRIC_SPECS)


SOURCE_ID = "eurostat"
_SOURCE_SET = frozenset({SOURCE_ID})
_DEFAULT_NAME = "Eurostat"
_DATASET_FIELDS = (
    "dataset_id",
    "table_name",
    "layer",
...
class EurostatBusinessCatalogError(ValueError):
    """Raised when EUROSTAT catalogue inputs do not match the release contract."""


def build_eurostat_business_catalog(
    *,
    code_sha: str,
    source_config: dict,
    ...
    if not isinstance(code_sha, str) or not code_sha.strip():
        raise EurostatBusinessCatalogError("code_sha must be a non-empty string")
    configured = _source_config(source_config)
    if SOURCE_ID not in configured:
        raise EurostatBusinessCatalogError("EUROSTAT source configuration must include eurostat")
    state = _source_state(ingestion_state)
    if any(source_id not in _SOURCE_SET for source_id in state):
        raise EurostatBusinessCatalogError("EUROSTAT ingestion state contains unknown sources")
    if not isinstance(dataset_metadata, list):
        raise EurostatBusinessCatalogError("dataset_metadata must be a list")
...
def _source_config(source_config: Any) -> Mapping[str, Any]:
    if not isinstance(source_config, Mapping):
        raise EurostatBusinessCatalogError("source_config must be an object")
    sources = source_config.get("sources", source_config)
    if not isinstance(sources, Mapping):
        raise EurostatBusinessCatalogError("source_config.sources must be an object")
...
def _source_state(ingestion_state: Any) -> Mapping[str, Any]:
    if not isinstance(ingestion_state, Mapping):
        raise EurostatBusinessCatalogError("ingestion_state must be an object")
    sources = ingestion_state.get("sources", ingestion_state)
    if not isinstance(sources, Mapping):
        raise EurostatBusinessCatalogError("ingestion_state.sources must be an object")
    return sources
...
def _build_source_entry(configured: Any, state: Any) -> dict[str, Any]:
    ...
    if not isinstance(raw_count, int) or isinstance(raw_count, bool) or raw_count < 0:
        raise EurostatBusinessCatalogError("eurostat raw_response_count must be a nonnegative integer")
    return result
...
def _build_lineage(dbt_manifest: Any, dataset_metadata: list[dict]) -> dict[str, list[dict[str, str]]]:
    if not isinstance(dbt_manifest, Mapping):
        raise EurostatBusinessCatalogError("dbt_manifest must be an object")
...
def _source_metrics(manifest: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for unique_id, record in sorted(metrics.items()):
        if not isinstance(record, Mapping) or _excluded(record, unique_id):
            continue
        if record.get("name") not in EUROSTAT_METRIC_NAMES:
            continue
