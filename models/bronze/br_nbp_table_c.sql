{{ config(materialized='table', alias='nbp_exchange_rates_table_c') }}

with batches as (
    select *
    from {{ source('landing', 'nbp_batches') }}
    where source_id = 'nbp_exchange_rates_table_c'
),
pages as (
    select
        source_id, batch_id, cast(ingestion_sequence as bigint) as ingestion_sequence,
        ...
        response_sha256, raw_file_id, body_json, publication.value as publication_json
    from batches
...
observations as (
    select
        ...
        retrieved_at_utc, response_sha256, raw_file_id,
        ...
        cast(json_extract_string(publication_json, '$.tradingDate') as date) as trading_date,
