...
**Repair in delivery:** `layout_resolution.py` previously classified the whole Drive root before selecting a source. Consequently, one unrelated legacy marker could stop every independent source writer after the verified canonical cutover. Writer resolution is now source-scoped: duplicate canonical containers and same-source canonical/legacy pointers still fail closed, while empty or unrelated legacy markers cannot create a platform-wide outage. NBP control resolution applies the same source-local rule. The 33-test Drive migration suite passes, including new regressions for an empty post-cutover wrapper and an active same-source dual-pointer conflict. Production acceptance remains pending a reviewed merge and fresh main-branch NBP, WDI, and Eurostat publication evidence.
