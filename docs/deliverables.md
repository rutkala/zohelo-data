...
Updated 13 September 2026. **Active priority: Production Google Drive Layout & Publication Audit (Issue #94).** The audit and selected NBP product acceptance are complete; full coverage of all four selected sources remains open. The owner specifically questioned the separate `bdl-platform` folder and release folder proliferation. Under the owner's sequential workflow, this task is the sole active engineering priority. Unrelated engineering is deferred while this audit and clarification are active; existing scheduled ingestion runs continue automatically on Actions.

### Active Priority: Production Google Drive Layout & Publication Audit (Issue #94) — 13 September 2026

- **Context and Owner Boundaries:** The owner questioned the separate `bdl-platform` folder, release folder proliferation, and physical medallion folders. In accordance with the sequential workflow, this is the single active task. No production data moves, renames, deletions, publishing, ingestion, or permission mutations are permitted in this increment. Existing ingestion schedules continue automatically on Actions without agent changes.
- **Verified in this Checkpoint:**
  - Codebase reconciliation: Documented why `src/bdl_platform.py:_release_root` creates `bdl-platform` and why `portal/src/services/googleDrive/releaseCatalog.ts` explicitly discovers both roots.
  - Owner guide: Added [docs/drive-structure.md](drive-structure.md) explaining how to find current data via pointers, physical vs logical layers in release folders, and why historical releases are retained under `retain_all_no_automatic_deletion` (compaction is on hold under H-CAP; rollback requires validation and is not instantaneous).
  - Reusable audit diagnostic: Implemented [src/drive_audit.py](../src/drive_audit.py) and CLI [scripts/audit_drive_structure.py](../scripts/audit_drive_structure.py) with a shared global budget across all requests/files/time, bounded media reads (capped at 1 MB) to prevent buffer exhaustion, protocol schema/scope/code_sha/status/test validation, and dataset/artifact metadata and parent verification without downloading data tables.
  - Fixture tests: Implemented [tests/test_drive_audit.py](../tests/test_drive_audit.py) (12 tests passing) covering global budget exhaustion, root and pointer ambiguity, bounded media downloads, protocol validation, missing current release directories, honest classification, and zero mutating Drive calls.
  - Minimal correction recommendation: Keep physical-versus-logical map and current pointers explicit; retain the existing two publication namespaces (`releases/` and `bdl-platform/releases/`); use measured audit evidence to choose later optimization. No speculative folder moves, ingestion gating implementation, policy change, or schedule pause in this checkpoint. A future migration requires exact old/new IDs, mapping all writers/readers, and serialized execution with rollback; it is not approved now.
- **Dated Baseline Evidence (13 September 2026 — live validation pending):**
  - Observation baseline from commit `0f68ad2`: 11 NBP release directories under `releases/` (newest: `e8c025a4-a7c7-432a-84fa-8151c1479c98`) and 79 BDL release directories under `bdl-platform/releases/` (newest: `668f721c-5f7e-446d-936f-e89745bb95d3`, 26 direct files totaling 435,547,793 bytes from metadata). These are baseline observations of specific folders only, not every release size or verified waste.
  - Pointers exist at root (`current-release.json`) and `bdl-platform/current-release.json`; live execution of the audit diagnostic and checksum verification remains pending controller execution.
  - Root `03_silver` and `04_gold` folders are unresolved historical (prototype paths); their live contents and empty status remain unverified until live execution. `promotion-audits` is an optional code-supported path not observed at root.
- **Explicit Remaining Work (Issue #94 remains OPEN):**
  - Controller execution of `scripts/audit_drive_structure.py` in an environment with live Drive read credentials.
  - Lead architectural review (Astra) of the audit results and future correction options before any production changes.
