...
Eurostat run [35464169768](https://github.com/rutkala/zohelo-data/actions/runs/35464169768) cleared the former layout failure: collection and the full 38-model/test dbt build succeeded. Promotion then failed closed while validating staged `eurostat_observation_revisions` because DuckDB's `HUGEINT` window-sum type is serialized to Parquet as `DOUBLE`, so the published bytes did not match source-relation metadata. The current release was retained. The model now casts revision sequence/count values to the lossless portable `BIGINT` contract, regression coverage verifies that its complete SQL schema survives the Parquet round trip, and validation errors identify the exact mismatched contract fields. A reviewed main-branch production rerun remains required before this repair is accepted. NBP's marked main-branch refresh was not started by PR #127 and likewise remains pending; neither pending acceptance is reported as complete.
