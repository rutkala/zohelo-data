> Historical proposal. Superseded by [the current architecture](../architecture.md); observations below describe the original audit baseline.
...
**Confirmed first interaction:** write SQL queries and view results. The owner selected “Write SQL queries and view results.” Prioritize the existing SQL editor, dataset/schema discovery and tabular query results for all already-ingested NBP data. The first demonstration should let the owner select a real dataset, run SQL and inspect the returned columns and rows. A failed load must clearly identify the failure; sample data or a previously selected dataset must not be presented as the requested dataset's result.
