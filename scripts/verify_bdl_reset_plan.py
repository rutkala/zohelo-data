#!/usr/bin/env python3
"""Verify that a BDL reset plan came from the exact successful reviewed workflow run."""
...
def canonical_plan_hash(plan: dict[str, Any]) -> str:
    ...
    if embedded and embedded != digest:
...
def verify_provenance(
    *,
    ...
    if (
        ...
        raise ProvenanceError("Reviewed plan pins do not match the reset dispatch")

    # In PR tests or without token, verify static pins only if token is empty
    if token:
        run = _json_request(
        ...
        if actual != required:
            raise ProvenanceError("Plan run is not the exact successful main reset workflow")
